...
class Contact(Person, Address):
    ...
    # return f"Я об'єкт типу {type(self)}, Мобільний номер контакту: {self.phone}"
...
class Notebok:
    ...
    def add_new_contact(self):
        ...
        """
        """"
        мене збиває що тут ми створюємо обюєкт контакт. Чому ми не можемо прочитати просто контакти.
          а створити змінніконтакту в інщому методі
        """

        #  ??? значення змінним(поля) що ініціалізуються в класі(person, adress) ми можемо надавати в іншому класі???
    ...
    def read_contact(self):
        ...
        for contact_dict in masiv_contact: # ??? як розуміти. Назва слоника одна, а словників до цієї назви декілька
            ...
            print(new_contact)  # ??? чому оце повертає нас в  def __str__(self). Як розуміти це?
            # Додаємо його до списку контактів до об'єкта Notebok
            self.list_of_contacts.append(new_contact)

    def _write_contacts_to_file(self):
        """
        В даному методі ми будемо записувати список контактів у json файл
        Зверни увагу, я зробив цей метод захищеним, це означає, що його небажано використовувати іншими програмістами
        Як ми будемо записувати дані у файл: У нас є список об'єктів Contact. Вони зберігаються
        у змінній self.list_of_contacts
        У файл нам треба записати масив словників
        Значить треба запустити ітерацію по нашому списку контактів
        далі створити словник із об'єкту Contact
        ...
        for contact in self.list_of_contacts:
            ...
            # Все інше в цьому методі спробуй реалізувати сам :)
            dict_contact = {
                "name": contact.name,
                "email": contact.email,
                "prizv": contact.prizv,
...
print(notebook.list_of_contacts[0])